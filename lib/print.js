...
const pathParams = function(ast) {
  ...
  const params = ast.params && buildEach(ast.params).join(' ');
...
const build = function(ast) {
  ...
  switch(ast.type) {
    case 'Program': {
      let inverse = ast.body[0];
      if(inverse && inverse.chained) {
        const chain = ast.body.shift();
      }
    ...
    case 'BlockStatement': {
      const lines = [];
      lines.push(['{{#', pathParams(ast), '}}', '\n'].join(''));
      lines.push(build(ast.program));
      if(ast.inverse) {
        lines.push(['{{', compactJoin(['else', pathParams(ast.inverse.body[0])],' '), '}}\n'].join(''));
        lines.push(build(ast.inverse));
      }
      lines.push(['{{/', build(ast.path), '}}'].join(''));
      lines.push('\n');
